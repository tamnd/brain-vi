...
```Bản dựng gỡ lỗi thường phơi bày lỗi sớm hơn. Việc sử dụng sai bộ nhớ dường như vô hại trong bản phát hành có thể bị hủy bỏ nhanh chóng trong bản dựng gỡ lỗi. 

Điều này rất hữu ích. Công việc nội bộ sẽ thất bại nặng nề. 

## 2.10 Kiểm tra cấu hình bản dựng 

CPython hiển thị cấu hình xây dựng thông qua`sysconfig`.```python
import sysconfig
...
|`make regen-all`| Tái tạo các tập tin được tạo | 
...
documentation tools
```Argument Clinic đặc biệt phổ biến trong phần mở rộng C và các định nghĩa phương thức tích hợp. Nó tạo mã phân tích cú pháp và trình bao bọc từ các nhận xét có cấu trúc. 
...
CPython có các móc gỡ lỗi cho bộ cấp phát bộ nhớ. 
...
| gỡ lỗi quá trình nhập bản dựng không khớp | chạy sai tệp thực thi hoặc sai`PYTHONPATH`| 

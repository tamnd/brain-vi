---
title: "CF 103230A - Torty và Shields"
...
[CF 103230A - Torty và Shields](https://codeforces.com/problemset/problem/103230/A) 
...
Cho phép$A_0,A_1,\dots,A_{m-1}$là đỉnh của$G_J$theo thứ tự tuần hoàn, trong đó$m=\binom{s+t}{t}$. Đối với mỗi$A_k$, xác định một bản sao của quá trình truyền tải hypercube$G_Q^{(k)}$TRÊN$Q_s$. Nếu như$k$là chẵn, đi qua$G_Q$theo hướng nhất định của nó; nếu như$k$thật kỳ quặc, đi ngang qua$G_Q$theo thứ tự ngược lại. Biểu thị chuỗi kết quả của chuỗi nhị phân bằng$X^{(k)}_0, X^{(k)}_1,\dots,X^{(k)}_{2^s-1}.$Xác định trình tự cấu hình chung bằng cách nối$$(A_k, X^{(k)}_0), (A_k, X^{(k)}_1), \dots, (A_k, X^{(k)}_{2^s-1})$$vì$k=0,\dots,m-1$. 

---
title: "CF 103637H - Giải vô địch khúc côn cầu"
...
[CF 103637H - Giải vô địch khúc côn cầu](https://codeforces.com/problemset/problem/103637/H) 
...
Một cách tiếp cận đơn giản sẽ thử tất cả các tập hợp con của các bài kiểm tra, nhưng$n$có thể lên tới$10^5$, vậy thậm chí$2^n$tập hợp con là không thể. Ngay cả việc hạn chế ở các tập hợp con nhỏ cũng không thành công vì cả thời gian và mức độ bao phủ bệnh tật đều tương tác với nhau trên toàn cầu. 
...
Điều này làm giảm vấn đề chọn tối đa một xét nghiệm cho mỗi bệnh, chọn ít nhất$k-1$bệnh tật và đảm bảo hạn chế về nguồn lực:$$\max t_i + \sum b_i \le T.$$Quan sát quan trọng tiếp theo là cố định thời gian tối đa. Giả sử chúng ta ấn định một ngưỡng$X$và chỉ xem xét các bài kiểm tra với$t_i \le X$. Đối với mỗi bệnh, chúng ta đương nhiên sẽ chọn xét nghiệm có sẵn với chi phí tối thiểu.$b_i$, vì việc sử dụng máu có tính bổ sung và độc lập đối với các bệnh. Sau khi chọn được những ứng viên tốt nhất, chúng ta cần quyết định xem ứng viên nào sẽ$k-1$bệnh tật phải giữ, và đó luôn là điều$k-1$chi phí máu nhỏ nhất trong số các bệnh hiện có. 
...
2. Duy trì một mảng`best[d]`được khởi tạo là “vô hạn”, thể hiện chi phí máu tốt nhất được tìm thấy cho bệnh tật cho đến nay$d$. 
...
Tính đúng đắn phụ thuộc vào cấu trúc đơn điệu: như$X$tăng lên, có nhiều xét nghiệm hơn và`best[d]`chỉ có thể giảm chứ không bao giờ tăng. Điều này đảm bảo chúng ta chỉ hướng tới các giải pháp tốt hơn hoặc tương đương trong khi quét. 
...
Một chi tiết cẩn thận là thành phần thời gian chỉ được đưa vào dưới dạng ngưỡng chung: nó không bao giờ cần được trộn vào chính cấu trúc heap. Sự tách biệt này là nguyên nhân làm cho việc giảm bớt việc quét đơn điệu trở nên khả thi. 

## Ví dụ đã hoạt động 

Xét một trường hợp nhỏ với$k=3$trong đó hai bệnh được xét nghiệm sớm đầy đủ và một bệnh cần sử dụng máu đắt tiền. Khi quá trình quét đạt đến ngưỡng thời gian mà tất cả các xét nghiệm liên quan đều có sẵn, thuật toán sẽ tích lũy mức tối thiểu cho mỗi bệnh và kiểm tra xem phương pháp nào là tốt nhất$k-1=2$bệnh phù hợp với quỹ máu cộng với thời điểm hiện tại. 
...
| Không gian |$O(n)$| lưu trữ để xét nghiệm và trạng thái của từng bệnh | 

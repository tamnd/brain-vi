---
title: "CF 103260B - Khóa cửa vs khách du lịch"
...
[CF 103260B - Khóa cửa và khách du lịch](https://codeforces.com/problemset/problem/103260/B) 
...
Để tạo tất cả các bảng, hãy sắp xếp các biến$x_{ij}$theo thứ tự từ điển$(x_{22},x_{23},\dots,x_{2n},x_{32},\dots,x_{mn}).$Mỗi$x_{ij}$có một khoảng hữu hạn các giá trị nguyên chấp nhận được xác định bởi tính không âm của$a_{i1}$Và$a_{1j}$. Các giới hạn này chỉ phụ thuộc vào các biến được chọn trước đó theo thứ tự từ điển, do đó là tập hợp các khả năng$(x_{ij})$là tích của các phạm vi số nguyên giới hạn ở dạng từ điển. 
...
Mỗi bước của quá trình này thay đổi chính xác một tọa độ$x_{ij}$qua$\pm 1$, do đó gây ra chính xác bốn thay đổi trong các mục ma trận tương ứng. Vì mọi bảng khả thi đều tương ứng duy nhất với một vectơ$(x_{ij})$, tất cả các bảng dự phòng đều được tạo và các bảng kế tiếp khác nhau ở đúng bốn mục. 

Điều này hoàn thành việc chứng minh. ∎ 

## Xác minh 

Việc xây dựng cây khung đảm bảo rằng mọi cạnh không phải là cây$(i,j)$với$i,j\ge 2$đóng một độc đáo$4$-đi qua$(i,1)$,$(1,1)$, Và$(1,j)$. Bất kỳ sự thay đổi đơn vị nào trong chu kỳ này đều bảo toàn tổng số hàng vì hàng$i$và hàng$1$mỗi người nhận được một mức tăng và một mức giảm. Điều tương tự cũng xảy ra với các cột$j$Và$1$. 

...
if __name__ == "__main__":
    solve()
```Các món ăn được sắp xếp để chúng ta có thể kích hoạt chúng trong một lần quét tuyến tính. Vùng heap lưu trữ các giá trị âm về độ ngon để mô phỏng vùng heap tối đa bằng cách sử dụng vùng heap tối thiểu của Python. Con trỏ$j$đảm bảo mỗi món ăn được chế biến đúng một lần. 
